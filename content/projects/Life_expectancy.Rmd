...
- ""
date: "2020-10-13"
description: change in life expectancy
draft: false
image: pic09.jpg
keywords: ""
slug: Expectancy
...
```{r,hiv_life_exp}
  ggplot(exp_hiv_worldbank,aes(x= Infection_Rate,y = Life_Expectancy, na.rm = TRUE))+
  ...
  labs(title = "Life Expectancy Likely Decreases with HIV Prevalence across Regions", 
       ...
       x = "Infection Rate", y = "Life Expectancy")
...
```{r,fertility_GDP_per_capita}
  ggplot(exp_hiv_worldbank,aes(x= Fertility_Rate,y = GDP_per_Capita, na.rm = TRUE))+
  geom_point(na.rm = TRUE, col = "black", cex = 0.5, alpha = 0.5)+
  facet_wrap(~region, ncol=3,nrow=3, scales = "free_y")+
  geom_smooth(method = "auto", na.rm = TRUE )+
  theme_gray()+
  labs(title = "GDP per Capita likely Decreases with Fertility Rates across Regions", subtitle = "GDP per Capita have large variations within regions, however", x = "Fertility Rate", y = "GDP per Capita")
...
ggplot(exp_hiv_worldbank,aes(x= Year, y = Mortality_Rate, na.rm = TRUE))+
  geom_point(na.rm = TRUE, col = "black", cex = 0.5, alpha = 0.5)+
  facet_wrap(~region, ncol=3,nrow=3, scales = "free_y")+
  geom_smooth(method = "auto", na.rm = TRUE )+
  theme_gray()+
  labs(title = "Mortality Decreased Over the Years in All Regions", subtitle = "North America sees the fastest improvement", x = "Year", y = "Mortality Rate")
...
```{r,school_fertility}
ggplot(exp_hiv_worldbank,aes(x= School_Enrollment, y = Fertility_Rate, na.rm = TRUE))+
  geom_point(na.rm = TRUE, col = "black", cex = 0.5, alpha = 0.5)+
  facet_wrap(~region, ncol=3,nrow=3, scales = "free_x")+
  geom_smooth(method = "lm", na.rm = TRUE )+
  theme_gray()+
  labs(title = "There is a Relatively Negative Relationship Between School Enrollment and Fertility Rate", subtitle = "The Relationship is Less Prominent in Europe and Central Asia", x = "School Enrollment", y = "Fertility Rate")
```
